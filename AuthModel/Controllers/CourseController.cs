...
namespace AuthModel.Controllers
{
    ...
    public class CourseController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetCourse(int id)
        {
            ...
            if (User.Identity.IsAuthenticated && await unitOfwork.UserCourseEnrollment.GetByIdAsync(a => a.UserId == User.FindFirstValue(ClaimTypes.NameIdentifier)) is not null)
            {
                var userProgress = await unitOfwork.UserCourseProgress.GetAsync(a => a.UserId == User.FindFirstValue(ClaimTypes.NameIdentifier) && a.CourseId == id);
                var userProgressDTOList = userProgress.Select(a => new UserCourseProgressResponseDTO()
                {
                    LessonId = a.LessonID,
                    WatchedDate = a.WatchedDate,
                }).OrderByDescending(a => a.WatchedDate).ToList();
                var obj = userProgressDTOList.FirstOrDefault();
                    if (obj != null)
                    returnedCourse.LastWatchedLessonID = obj.LessonId;   //userProgressDTOList[0].LessonId;
                returnedCourse.UserCourseProgress = userProgressDTOList;
            }
        ...
        public async Task<IActionResult> GetLesson(int courseId, int lessonId)
        {
            ...
            if (await unitOfwork.Course.GetByIdAsync(a => a.Id == courseId) is null)
                return NotFound(new { message = "The Course Not Found" });
            //validate if the lesson is found and related to this course id
            var lesson = await unitOfwork.Lesson.GetByIdAsync(a => a.Id == lessonId && a.Section.CourseId == courseId);
            if (lesson is null)
                return NotFound(new { message = "The Lesson Not Found" });
            if (await unitOfwork.UserCourseEnrollment.FindAnyAsync(a => a.UserId == User.FindFirstValue(ClaimTypes.NameIdentifier) && a.CourseId == courseId) is null)
                return BadRequest(new { message = "You Must Enroll The Course First" });

            var progress = await unitOfwork.UserCourseProgress.GetByIdAsync(a => a.LessonID == lessonId &&
            a.CourseId == courseId && a.UserId == User.FindFirstValue(ClaimTypes.NameIdentifier));
            if (progress is null)
            {
                await unitOfwork.UserCourseProgress.AddAsync(new UserCourseProgress()
                {
                    CourseId = courseId,
                    LessonID = lessonId,
                    UserId = User.FindFirstValue(ClaimTypes.NameIdentifier) ?? "none",
            ...
            else
            {
                ...
                unitOfwork.UserCourseProgress.Edit(progress);
            }    
            await unitOfwork.SaveDataAsync();   
